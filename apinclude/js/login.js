...
$('#but_login').click(function(e){
    ...
    {
        showError("INGRESE RUT / CONTRASEÑA",900);
        $('.inner').jrumble({
            x: 4,
            y: 0,
            rotation: 0
        ...
        $.ajax({
            ...
            success: function(datos){
                if(datos == "1"){
                    hideTop();
                    loading('Conectando',1);		
                    setTimeout( "unloading()", 2000 );
                    setTimeout( "Login()", 2500 );
                    return true;
                }else{
                    showError("INGRESAR RUT O CONTRASEÑA VALIDA", 100);
                    $('.inner').jrumble({
                        x: 4,
                        y: 0,
                        rotation: 0
...
function Login(){
    ...
    setTimeout( "window.location.href='index.php#inicio'", 1000 );
}
