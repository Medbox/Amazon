...
session_start();
$evolucion     = strtoupper($_GET["evolucion"]);
$fic_id        = $_GET["fic_id"];
...
if (is_numeric($fic_id)) {
    if (count($arr) < 1) {
        $sql_fic = $obj_cie->agregarCierre(null, $fic_id, $evolucion, $cie_usu, $cie_ip);
        ...
        $sql_fic = $obj_cie->agregarCierre($arr[0]['cie_id'],$fic_id, $evolucion, $cie_usu, $cie_ip);
