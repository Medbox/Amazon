...
$arr_con	=	$obj_con->list_contrato($fun_id);

var_dump($arr_con);

/*if(count($arr_con) != 1){
	
	header("Location: woc.php");
...
(
    [0] => Array
        (
            [fun_rut] => 15987066-9
            [nombre] => CLAUDIO HERMOSILLA DIAZ
            [con_cargo] => Jefe del departamento de ProgramaciÃ³n, en su calidad de Ingeniero InformÃ¡tico; ejerciendo el cargo de direcciÃ³n de las personas que bajo su cargo se encuentren
            [con_ciudad] => comuna y ciudad de Temuco
            [con_civil] => Casado
            [con_dep] => PROGRAMACION
            [con_domicilio] => Hochstetter 355 Departamento 42
            [con_mail_revolt] => xcoltx@gmail.com
            [con_ocupacion] => Ingeniero InformÃ¡tico
            [con_renta] => 500.000 mil pesos (quinientos mil pesos)
            [con_orden] => 1
        )
)
*/
...
$DATA_AUX		=	explode("-",$arr_con[0]["fun_rut"]);//$ARR_CONTRATO[$fun_id]['RUT'];//"15.123.123-4";
$DATA_RUT		=	number_format($DATA_AUX[0],0,"",".").'-'.$DATA_AUX[1];

$DATA_OCUPACION	=	$arr_con[0]["con_ocupacion"];//$ARR_CONTRATO[$fun_id]['OCUPACION'];//"Profesional";
$DATA_CIVIL		=	$arr_con[0]["con_civil"];//$ARR_CONTRATO[$fun_id]['CIVIL'];//"Casado";
$DATA_DOMICILIO	=	$arr_con[0]["con_domicilio"];//$ARR_CONTRATO[$fun_id]['DOMICILIO'];//"Hochstetter 355 Dep. 42";
$DATA_CIUDAD	=	$arr_con[0]["con_ciudad"];//$ARR_CONTRATO[$fun_id]['CIUDAD'];//"comuna y ciudad de Temuco";
$DATA_CARGO		=	$arr_con[0]["con_cargo"];//$ARR_CONTRATO[$fun_id]['CARGO'];//"Jefe Programación";
$DATA_RENTA		=	$arr_con[0]["con_renta"];//$ARR_CONTRATO[$fun_id]['RENTA'];//"800.000 mil pesos (Ochocientos mil pesos)";
