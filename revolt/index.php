...
<link href="plugins/jquery-ui/themes/base/jquery-ui.css" rel="stylesheet" type="text/css">
</head>
<body>
<style>
html,body{
...
$(document).ready(function(e) {
	...
	$("section").on("click","#btn_crear_ficha",function(e){
		...
		var fun_id		=	$("#hi_fun_id").val();
		var nom_prog	=	$("#ipt_nombre_programa").val();
		var conductor	=	$("#ipt_conductor").val();		
		var bloque		=	$("#ipt_bloque_contenido").val();
		var programa	=	$("#ipt_numero_programa").val();
		var linea_edit	=	$("#txa_editorial").val();		
		var juegos		=	$("#txa_juegos").val();
		var desarrollo	=	$("#txa_programa").val();		
		
		var flag		=	true;
		
		if(fun_id == ""){						alert("NO_SESSION");					flag = false;	}
		if(nom_prog 	== "" && flag == true){	alert("Falta el Nombre del Programa");	flag = false;	}
		if(conductor	== "" && flag == true){	alert("Falta el Nombre del Conductor");	flag = false;	}		
		if(bloque		== "" && flag == true){	alert("Bloque no Seleccionado");		flag = false;	}
		if(programa		== "" && flag == true){	alert("Programa no Seleccionado");		flag = false;	}
		if(linea_edit	== "" && flag == true){	alert("Linea Editorial no completada");	flag = false;	}		
		if(juegos 		== "" && flag == true){	alert("No se han espeficicado los juegos a desarrollar");	flag = false;	}		
		if(desarrollo 	== "" && flag == true){	alert("Debes explicar el desarrollo de tus programas");		flag = false;	}

		if(flag == true){
			
			var conf	=	confirm("Vas a crear una ficha de registro mensual, desas continuar?");
			
			if(conf == true){
				
				var url_get	=	"option=AM";
					url_get	+=	"&fun_id="+fun_id;
					url_get	+=	"&conductor="+conductor;
					url_get	+=	"&bloque_programa="+bloque;
					url_get	+=	"&numero_programa="+programa;
					url_get	+=	"&linea_editorial="+linea_edit;
					url_get	+=	"&desarrollo_programa="+desarrollo;
					url_get	+=	"&nombre_programa="+nom_prog;
					url_get	+=	"&juegos="+juegos;
